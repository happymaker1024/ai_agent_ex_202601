...
# langgraph_ 디렉토리를 Python 경로에 추가
sys.path.insert(0, str(Path(__file__).parent / 'langgraph_'))
...
def test_langgraph_agent():
    ...
    try:
        print("\n" + "="*60)
        print("LangGraph Agent Test Started")
        print("="*60 + "\n")

        # LangGraph 시스템 초기화
        graph_system = InvestmentReportingGraph(api_key)
        print("[OK] LangGraph system initialized")

        # Samsung Electronics analysis
        company = "Samsung Electronics (005930)"
        print(f"\nAnalyzing: {company}\n")

        result = graph_system.run_analysis("삼성전자")
